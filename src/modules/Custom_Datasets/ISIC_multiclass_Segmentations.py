...
class ISIC_multiclass_Segmentations(torch.utils.data.Dataset):
    def __init__(self, mode='test', basepath=None, transforms=None, seed=42):
        ...
        if mode == 'train':
            self.datafile = os.path.join(self.basepath, 'ISIC/ISIC_Challenge_Dataset/datadings/Segmentations/multiclass/train.msgpack')
        elif mode == 'val':
            self.datafile = os.path.join(self.basepath, 'ISIC/ISIC_Challenge_Dataset/datadings/Segmentations/multiclass/val.msgpack')
        elif mode == 'test':
            self.datafile = os.path.join(self.basepath, 'ISIC/ISIC_Challenge_Dataset/datadings/Segmentations/multiclass/test.msgpack')
