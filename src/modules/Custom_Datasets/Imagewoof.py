...
class Imagewoof(torch.utils.data.Dataset):
    def __init__(self, mode='test', basepath=None, transforms=None, seed=42):
        ...
        self.class_names = ["Australian_terrier", "Border_terrier", "Samoyed", "Beagle", "Shih-Tzu", "English_foxhound", "Rhodesian_ridgeback", "Dingo", "Golden_retriever", "Old_English_sheepdog"]
        self.num_classes = len(self.class_names)

        self.lock = multiprocessing.Lock()

        if mode == 'train':
            self.datafile = os.path.join(self.basepath, f'Imagewoof/datadings/train.msgpack')
        elif mode == 'val':
            self.datafile = os.path.join(self.basepath, f'Imagewoof/datadings/val.msgpack')
        elif mode == 'test':
            self.datafile = os.path.join(self.basepath, f'Imagewoof/datadings/test.msgpack')
