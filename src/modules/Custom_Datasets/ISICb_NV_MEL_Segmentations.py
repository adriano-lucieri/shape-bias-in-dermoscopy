...
class ISICb_NV_MEL_Segmentations(torch.utils.data.Dataset):
    def __init__(self, mode='test', basepath=None, transforms=None, seed=42, portion=None):
        ...
        if mode == 'train':
            if portion is None:
                self.datafile = os.path.join(self.basepath, 'ISIC/datadings/Segmentations/binary_NV-MEL/train.msgpack')
            else:
                target_file = os.path.join(self.basepath, f'ISIC/datadings/Segmentations/binary_NV-MEL/train-{portion}.msgpack')
                print(f'Trying to load {target_file}')
                self.datafile = target_file
        elif mode == 'val':
            self.datafile = os.path.join(self.basepath, 'ISIC/datadings/Segmentations/binary_NV-MEL/val.msgpack')
        elif mode == 'test':
            self.datafile = os.path.join(self.basepath, 'ISIC/datadings/Segmentations/binary_NV-MEL/test.msgpack')
