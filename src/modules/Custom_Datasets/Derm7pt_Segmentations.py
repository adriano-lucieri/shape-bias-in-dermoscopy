...
class Derm7pt_Segmentations(torch.utils.data.Dataset):
    def __init__(self, split='test', target_label='diagnosis', basepath=None, transforms=None, seed=42):
        ...
        if split == 'train':
            self.datafile = os.path.join(self.basepath, f'Derm7pt/datadings/Segmentation/Derm7pt_strat_perTarget/Derm7pt_strat_{self.target_label}/train.msgpack')
        elif split == 'val':
            self.datafile = os.path.join(self.basepath, f'Derm7pt/datadings/Segmentation/Derm7pt_strat_perTarget/Derm7pt_strat_{self.target_label}/val.msgpack')
        elif split == 'test':
            self.datafile = os.path.join(self.basepath, f'Derm7pt/datadings/Segmentation/Derm7pt_strat_perTarget/Derm7pt_strat_{self.target_label}/test.msgpack')
