...
class Imagenette(torch.utils.data.Dataset):
    def __init__(self, mode='test', basepath=None, transforms=None, seed=42):
        ...
        self.class_names = ["tench", "English_springer", "cassette_player", "chain_saw", "church", "French_horn", "garbage_truck", "gas_pump", "golf_ball", "parachute"]
        self.num_classes = len(self.class_names)

        self.lock = multiprocessing.Lock()

        if mode == 'train':
            self.datafile = os.path.join(self.basepath, f'Imagenette/datadings/train.msgpack')
        elif mode == 'val':
            self.datafile = os.path.join(self.basepath, f'Imagenette/datadings/val.msgpack')
        elif mode == 'test':
            self.datafile = os.path.join(self.basepath, f'Imagenette/datadings/test.msgpack')
