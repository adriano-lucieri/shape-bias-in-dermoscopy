...
from utils.utils import make_dir_if_not_exists, save_evaluation_results, average_n_run_results_eval, setup_default_logging
...
def evaluate(config, target_experiment_dir):
    ...
    model_dir = os.path.join(DIR_RESULTS, target_experiment_dir, config['experiment_parameters']['run_comment'])
    ...
    dir_results = os.path.join(model_dir, config['dataset_parameters']['eval_dataset'].replace('_', '-') + eval_dataset_suffix +  '-DFT')
    ...
    eval_dataset_suffix = config['dataset_parameters']['target_augmentation']
    eval_dataset_suffix = '_' + eval_dataset_suffix if eval_dataset_suffix != '' else ''
    model_path = os.path.join(model_dir, f'best_checkpoint{eval_dataset_suffix}.pth')
...
def train_evaluate(trial, config, train_args, target_experiment_dir):
    ...
    #######
    # Get already trained model from base_experiment_dir
    #######
    ...
    ### Load trained weights, freeze base and reinitialize classifier
    logger.info('Loading model from %s'%best_checkpoint_path)
    ...
    model.fc = nn.Linear(model.fc.in_features, int(config['dataset_parameters']['num_classes']))
    ...
    dir_results = os.path.join(DIR_RESULTS, target_experiment_dir, config['experiment_parameters']['run_comment'])
    best_checkpoint_path = os.path.join(dir_results, f'best_checkpoint{eval_dataset_suffix}.pth')
    #last_checkpoint_path = os.path.join(dir_results, f'last_checkpoint{eval_dataset_suffix}.pth')
    make_dir_if_not_exists(dir_results)
...
if __name__ == '__main__':
    ...
    # Modify experiment dir to target_experiment_dir so that original doesn't get overwritten.
    target_experiment_dir = config['experiment_parameters']['exp_dir'].split('/')
    target_experiment_dir[0] = target_experiment_dir[0] + '-DFT'
    target_experiment_dir = '/'.join(target_experiment_dir)

    logger, output_dir = setup_default_logging(os.path.join(DIR_RESULTS, target_experiment_dir))
    logger.info(options)
    logger.info("")
    logger.info("#######################")
    ...
    if config['experiment_parameters']['mode'] == 'test':
        ...
        for run_idx in range(options.random_runs_start_from, options.random_runs):
            ...
            # Normal Training
            train_evaluate(None, config, train_args, target_experiment_dir)

            evaluate(config, target_experiment_dir)

        eval_dataset_suffix = config['dataset_parameters']['target_augmentation']
        eval_dataset_suffix = '_' + eval_dataset_suffix if eval_dataset_suffix != '' else ''

        average_n_run_results_eval(
            target_folder=os.path.join(DIR_RESULTS, target_experiment_dir), 
            target_dataset=config['dataset_parameters']['eval_dataset'].replace('_', '-') + eval_dataset_suffix, 
            num_runs=options.random_runs,
            run_comment=base_run_comment
        )
